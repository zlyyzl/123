...
def prediction_page():
    ...
    if page == "Home Page":
        ...
        if prediction_type == "Preoperative_number":
            ...
            if st.button('Add Data for Learning'): 
                if len(input_df) > 0:  # 确保有数据
                    model.fit(input_df, np.array([label]))  # 使用NumPy数组避免潜在问题
                    st.success("New data has been added to the model for continuous learning!")
                else:
                    st.error("Input data is not valid for learning.")
    ...
    # 这里不需要将 procedural_time 放入列表中
            ...
            if st.button('Add Data for Learning'): 
                if len(input_df) > 0:  # 确保有数据
                    model3.fit(input_df, np.array([label]))  # 使用NumPy数组避免潜在问题
                    st.success("New data has been added to the model for continuous learning!")
                else:
                    st.error("Input data is not valid for learning.")
